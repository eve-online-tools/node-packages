...
import { compatibilityDateExists, readCompatibilityDate } from '../lib/compatibility-date-file'
import { fetchOpenApiSpec } from '../lib/esi-fetch'
import { ensureIndexTs, generateSchema, writeOpenApiSpec } from '../lib/output-files'
...
export const downloadSpec = async ({ outputDir, updateOnly = false }: DownloadSpecOptions): Promise<void> => {
  ...
  if (updateOnly) {
    ...
    if (!changed) {
      console.log(`Compatibility date unchanged (${current}). Skipping spec download.`)
      return
    }
  ...
  if (!compatibilityDate) {
    throw new Error(`Missing compatibility date in ${outputDir}. Run update-compatibility-date first.`)
  }

  const spec = (await fetchOpenApiSpec(compatibilityDate)) as Record<string, unknown>
  ...
  const { created, path: indexPath } = ensureIndexTs(outputDir, spec, esiBaseUrl)
