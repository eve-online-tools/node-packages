export const sortedDict = (_key: string, value: unknown): unknown =>
...
export const formatSpec = (spec: unknown): string => `${JSON.stringify(spec, sortedDict, 2)}\n`
...
const PARAM_REFS_TO_STRIP = new Set(['#/components/parameters/CompatibilityDate', '#/components/parameters/Tenant'])
...
const shouldStripParameter = (param: OpenApiParameter): boolean => {
  ...
  return param.in === 'header' && !!param.name && HEADER_NAMES_TO_STRIP.has(param.name)
}
...
const stripParametersArray = (item: { parameters?: unknown[] }): void => {
  ...
  item.parameters = item.parameters.filter((param) => !shouldStripParameter(param as OpenApiParameter))
}
