...
describe('stripGlobalHeaders', () => {
  it('removes compatibility and tenant parameter refs and inline headers', () => {
    ...
    expect(spec.paths['/status/'].parameters).toEqual([{ name: 'datasource', in: 'query' }])
    expect(spec.paths['/status/'].get.parameters).toEqual([{ name: 'language', in: 'query' }])
  })
})
...
describe('extractBaseUrlFromSpec', () => {
  it('reads servers[0].url from the spec', () => {
    expect(
      extractBaseUrlFromSpec({ servers: [{ url: 'https://esi.evetech.net' }] }, 'https://fallback.example'),
    ).toEqual({
...
describe('generateIndexTs', () => {
  it('includes editable banner, baseUrl, createESIProvider helper, and type re-exports', () => {
    const content = generateIndexTs({ servers: [{ url: 'https://esi.evetech.net' }] }, 'https://fallback.example')
    ...
    expect(content).toContain('export type { paths, components, operations } from "./esi-schema"')
