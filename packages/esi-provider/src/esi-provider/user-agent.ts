 ...
 */
export const buildUserAgent = (appName: string, appVersion: string, appContact: string | Array<string>) => {
  if (!appName) {
  ...
  const appContactString = Array.isArray(appContact) ? appContact.join('; ') : appContact
