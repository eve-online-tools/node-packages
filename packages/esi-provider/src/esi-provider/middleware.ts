...
import type { CombinedMiddlewareState, Middleware, MiddlewareContext, RootAction } from './types'
...
export const combineInitialState = (middleware: Array<Middleware>): CombinedMiddlewareState => {
...
export const wrapMiddleware = <TState>(
  ...
  return {
    onRequest: onRequest ? (options) => onRequest({ ...options, ...ctx }) : undefined,
    onResponse: onResponse ? (options) => onResponse({ ...options, ...ctx }) : undefined,
