import { type PropsWithChildren, useContext, useMemo, useReducer, useRef } from 'react'
import createClient, { type Client } from 'openapi-fetch'
import { esiBaseUrl, esiCompatibilityDate, esiTenant } from './config'
import { context } from './context'
import { combineInitialState, createRootReducer, wrapMiddleware } from './middleware'
import { buildUserAgent } from './user-agent'
import type { Middleware } from './types'

export type PropsWithDefaults<TProps, TDefaults extends Partial<TProps>> = Omit<TProps, keyof TDefaults> &
 ...
 */
export const createESIProvider = <Paths extends {}>(props: NewProviderProps): CreateProviderResult<Paths> => {
  const { baseUrl, compatibilityDate, fetch, tenant, appName, appVersion, appContact, middleware } = {
    ...defaultProps,
    ...props,
  ...
  const Provider = ({ children }: PropsWithChildren) => {
    ...
    const client = useMemo(() => {
      ...
      const wrappedMiddleware = middleware.map((mw) =>
        wrapMiddleware(mw, {
          ...
          getState: () => (stateRef.current[mw.key] as typeof mw.initialState) ?? mw.initialState,
        }),
      )
    ...
    return <context.Provider value={{ client, middleware, state }}>{children}</context.Provider>
  }
