...
export const parseRateLimitLimitHeader = (value: string | null): ParsedRateLimitLimit | null => {
  if (!value) {
    return null
  }
  ...
  if (!Number.isFinite(maxTokens) || !Number.isFinite(windowAmount) || maxTokens <= 0 || windowAmount <= 0) {
    return null
  }
...
export const estimateBucketUsage = (bucket: RateLimitBucket, now = Date.now()): BucketUsage => {
...
export const buildRateLimitsView = (state: RateLimitState, now = Date.now()): RateLimitsView => {
  ...
  const groups = Array.from(groupMap.values())
    .map((groupView) => ({
      ...
      buckets: [...groupView.buckets].sort((a, b) => a.rateLimitKey.localeCompare(b.rateLimitKey)),
    }))
...
export const toBucketView = (bucket: RateLimitBucket, rateLimitKey: string, now = Date.now()): RateLimitBucketView => {
  const usage = estimateBucketUsage(bucket, now)
  return {
    rateLimitKey,
    remaining: bucket.remaining,
